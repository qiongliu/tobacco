<template>
  <div>
    <div class="top">
      <div class="top_first">
        <span class="line"></span> 权限管理
      </div>
      <div class="top_sen">
        <el-input placeholder="请输入内容" v-model="searchData.str" size="small">
          <template slot="append">
            <i class="iconfont icon-sousuo" @click="findPrmissStr()"></i>
          </template>
        </el-input>
      </div>
      <div class="top_excel">
    ...
    <el-table :data="tableData" style="width: 100%" :header-cell-style="getRowClass">
      ...
      <el-table-column prop="name" label="岗位类型"></el-table-column>
      <el-table-column prop="details" label="权限"></el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <i class="iconfont icon-bianji" @click="updatePremiss(scope.row.id)"></i>
          <i class="iconfont icon-shanchu" @click="deletePremiss(scope.row.id)"></i>
        </template>
      </el-table-column>
    </el-table>
    <div class="page">
...
export default {
  ...
  methods: {
    ...
    findAllPremiss(){
      Management.findAllPremiss(this.searchData.str,this.searchData.pageNumber,this.searchData.currentPage).then(res => {
        if (res === true) return;
    ...
    // 导出表格
    exportExcel() {
      Management.exportRoleExcel().then(res => {
    ...
    // 删除
    deletePremiss(id) {
      Management.deletePremiss(id).then(res => {
        this.$message({
          message: "删除成功",
          type: "success"
        });
