<template>
  <div>
    ...
    <el-table :data="tableData" style="width: 100%" :header-cell-style="getRowClass">
      ...
      <el-table-column prop="type" label="推送类型"></el-table-column>
      <el-table-column prop="data" label="推送内容"></el-table-column>
       <el-table-column prop="area.name" label="推送对象"></el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <!-- <i class="iconfont icon-bianji" @click="updatePushObj(scope.row.id)"></i> -->
          <i class="iconfont icon-shanchu" @click="deletePushlist(scope.row.id)"></i>
        </template>
      </el-table-column>
    </el-table>
    <div class="page">
  ...
  methods: {
    ...
    getPushhisList() {
      Management.getPushhisList(
       ...
       exportExcel() {
      Management.exportPushlishExcel().then(res => {
    ...
    // },
    deletePushlist(id) {
      Management.deletePushlist(id).then(res => {
        this.$message({
          message: "删除成功",
          type: "success"
        });
