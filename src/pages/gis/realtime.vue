<template>
  <div class="m-realtime">
    ...
    <div class="realtime">
      ...
      <m-select class="feature-select" @click="selectHandle"></m-select>
      <p class="f-gis-sub-title">
        各站点气温要素信息
      </p>
      <div class="date">
        <span>最新</span>
        <el-date-picker v-model="allStationTime" @change="allStationTimeChange" format="yyyy-MM-dd HH:00" value-format="yyyyMMddHH" type="datetime" placeholder="选择日期时间">
        </el-date-picker>
      </div>
      <div class="all-station-chart">
        <ul>
          <li v-for="(item,index) of allStationData" :key="index"><span>{{item.station_name}}</span><el-progress class="progress" :show-text="false" :percentage="calculateScale(item[weatherFeature])"></el-progress><span>{{item[weatherFeature]}} {{featureUnit[weatherFeature]}}</span></li>
        </ul>
      </div>
      <div class="single-station">
        <p>{{currentSation.station_name}}{{feature2Name[weatherFeature]}}变化趋势</p>
        <div class="single-station-time">
          <div class="week-24">
            <span :class="weekOr24 === '24' ? 'active' : ''" @click="weekOr24Handle('24')">24h</span>
            <span>|</span>
            <span :class="weekOr24 === 'week' ? 'active' : ''" @click="weekOr24Handle('week')">本周</span>
          </div>
          <el-date-picker v-model="singleStationTime" @change="onChangeSingleStationTime" type="datetimerange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" format="yyyy-MM-dd HH:00" value-format="yyyyMMddHH">
          </el-date-picker>
        </div>
    ...
    methods: {
      getAllStations() {
        ...
        Gis.getAllStations(this.allStationTime).then(res => {
          ...
          this.currentSation = res.find(item => {
      ...
      selectHandle(val) {
        this.weatherFeature = val
        this.renderSigleStationRealtime()
        this.$emit('weatherFeature', val)
      },
      weekOr24Handle(val) {
        this.weekOr24 = val
      },
      ...
      renderSigleStationRealtime() {
        this.showLoading()
        Gis.getSigleStationRealtime(this.currentStationId, this.singleStationTime[0], this.singleStationTime[1]).then(res => {
          let dataset = [['time', 'tem', 'rhu', 'prs', 'pre_1h', 'win_s', 'win_d']]
          res.forEach(item => {
            dataset.push([
