...
export default {
  ...
  data () {
    return {
      ...
      lastType: '',
      sketchListener: null,
      sketch: null, // @type {Feature} Currently drawn feature.
  ...
  methods: {
    _load () {
      ...
      this.render(TYPE, new VectorSource({ crossOrigin: 'anonymous' }), this._getStyle());
    ...
    _getGeometryFunction () {
      ...
      if (this.type === 'Square') {
        ...
        //   let geometry = optGeometry || Polygon.fromCircle(new Circle(center, radius, 'XY'), 5, 0);
        //   return geometry;
        // };
      }
    ...
    _getStyle () {
      let defaultStyle = [
        new Style({
          stroke: new Stroke({ color: '#fff', width: WIDTH + 2 })
        }),
        new Style({
          image: new Circle({
            radius: WIDTH * 2,
          ...
          stroke: new Stroke({ color: '#0099FF', width: WIDTH }),
          fill: new Fill({
    ...
    _uninitMeasure () {
      ...
      this.sketchListener && unByKey(this.sketchListener);
    },
    _registerEvents () {
    ...
    clearSource () {
      this.layer && this.layer.getSource() && this.layer.getSource().clear();
    }
  },
  beforeDestroy () {
    this.clearSource();
