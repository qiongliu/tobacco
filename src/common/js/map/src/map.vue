...
import { defaults as controlDefaults } from 'ol/control/util';
import ScaleLineCtrl from 'ol/control/ScaleLine';
...
export default {
  ...
  watch: {
    zoom (newZoom) {
      if (newZoom === this.map.getView().getZoom()) { return false; }
      this.map && this.map.getView() && this.map.getView().setZoom(newZoom);
    },
    center (newCenter) {
      this.map && this.map.getView() && this.map.getView().setCenter(transform(newCenter, 'EPSG:4326', 'EPSG:3857'));
    },
    layers (newLayers) {
      if (newLayers.length) {
  ...
  methods: {
    ...
    _initControl () {
      this.scaleLine && this.map.addControl(new ScaleLineCtrl());
    },
    _initMapEvent () {
      ...
      view.on('change:resolution', debounce(DEBOUNCE_TIME, (event) => {
    ...
    _addClickInteraction () {
      this.click = new SelectInteraction({
        style: (feature) => {
    ...
    _addHoverInteraction () {
      this.hover = new SelectInteraction({
        ...
        style: (feature) => {
          // let f = getSingleFeature(feature);
          return getSingleFeature(feature).get('style');
        }
      });
